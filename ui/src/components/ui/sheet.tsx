...
const SheetOverlay = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof SheetPrimitive.Overlay>>(
  ({ className, ...props }, ref) => (
    <SheetPrimitive.Overlay
      ref={ref}
      className={cn(
        'fixed inset-0 z-50 bg-black/30 backdrop-blur-md data-[state=open]:animate-in data-[state=open]:fade-in data-[state=closed]:animate-out data-[state=closed]:fade-out',
        className
      )}
      {...props}
    />
  ...
  ({ className, children, side = 'right', ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ref={ref}
        className={cn(
          'fixed z-50 flex h-full w-full flex-col border border-border bg-surface-2 p-6 text-text shadow-soft transition ease-elegant data-[state=open]:animate-in data-[state=closed]:animate-out sm:max-w-lg',
